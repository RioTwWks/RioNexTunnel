...
**RioNexTunnel** спроектирован так, чтобы быть невосприимчивым к этому классу уязвимостей.
...
## Pinning сертификата подписки (опционально)

По умолчанию загрузка подписки использует обычную проверку TLS (системные CA). В **Настройки → Advanced security** можно включить **SPKI certificate pinning**.
...
| Сопоставление хоста | Pin сохраняются по hostname подписки (без учёта регистра) |
| Проверка | При включении проверяются только хосты с сохранёнными pin; остальные — обычный TLS |
| Panel API | HTTP RioNexGate **не** pin-ится (отдельный код) |

### Ограничения

- При смене TLS-сертификата панели (например, продление Let's Encrypt) нужно **обновить pin**, иначе обновление подписки завершится ошибкой mismatch.
- Для pinned-хостов CA-доверие заменяется сохранённым SPKI pin.
- Pinning **не поддерживается** в web-сборках.
- Pin и учётные данные **не попадают в логи**.

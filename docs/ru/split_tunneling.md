...
**Статус:** отложено. Сейчас Linux desktop работает только в **proxy mode** (`VpnMode.proxy`). Нет TUN-интерфейса и per-app маршрутизации на уровне ОС.

При появлении **Linux TUN mode** потребуется:

| Требование | Зачем |
|------------|-------|
| Policy routing (`ip rule` / `nftables`) или cgroups v2 | Маршрутизация только выбранных приложений через туннель |
| Блок обхода через localhost | Приложения не должны достигать неаутентифицированный SOCKS на `127.0.0.1` |
| Интеграция с kill switch | Падение туннеля без утечек |
| Отдельно от proxy mode | В proxy mode split tunnel остаётся на уровне ОС/приложений |

Пункт **Linux TUN split tunnel** в `tasks.md` — только документация, пока TUN не реализован.
